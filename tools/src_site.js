/* Render de Martes — 2026
   Reveals, scrollspy, muted looping trailers, bone cursor trail. Vanilla, no deps. */
(function () {
  'use strict';
  ...
  /* ---------- galleries + muted autoplay loops ---------- */
  ...
  Array.prototype.forEach.call(galleries, function (gal) {
    ...
    // arrows: same step as a swipe, wrapping at both ends
    function step(dir) {
    ...
    // touch: swipe the media itself. Horizontal intent only — a mostly-vertical
    // drag is the page scrolling and must not steal the gesture.
    if (slides.length > 1) {
  ...
  /* ---------- click-to-play GIFs ----------
     Workflow pages carry ~95 MB of animation. Each heavy GIF ships as a first
     frame poster and only fetches the real file when someone asks for it. */
  document.addEventListener('click', function (ev) {
  ...
  /* ---------- workflow clips ----------
     Former GIFs, now muted H.264. preload="none" until they are near the
     viewport, then play; paused again once they leave, so a long page never
     decodes twenty videos at once. */
  ...
  /* ---------- spam-safe mail buttons ----------
     The address never appears in the markup: it is assembled from data-u / data-d
     at click time, so scrapers that read the HTML find nothing to harvest. */
  document.addEventListener('click', function (ev) {
  ...
  // reveal the address on hover/focus so people can see where it goes before clicking
  ...
  /* ---------- bone cursor trail (rewritten) ----------
     A rAF loop walks from the last emitted point toward the live pointer and drops a
     bone every STEP pixels, so spacing comes from distance travelled, never from event
     frequency or a timer. Every bone gets its own rotation, size, tumble and drift. */
  ...
  // the pointer left the window: forget where we were so re-entry does not streak
